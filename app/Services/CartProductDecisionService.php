...
PROMPT;
     ...
     */
    private function preparationPrompt(
        array $eventContext,
        array $shoppingPlan,
    ): string {
        $prompt = <<<'PROMPT'
...
PROMPT;
    ...
    private function preparationNamingRules(): string
    {
        return <<<'PROMPT'
- name є першим прямим запитом у каталог і називає конкретну куповану харчову ідентичність та придатну форму, а не страву, рецепт чи абстрактне призначення;
- якщо погоджені name, note або контекст описують спосіб приготування чи майбутню страву, ДО пошуку використай кулінарні знання й визнач, який конкретний сирий або готовий до купівлі харчовий продукт людина зазвичай кладе в кошик для цього; не копіюй фразу про призначення як назву товару;
- збережи прямо названий вид мʼяса, риби чи іншого продукту, сирий або оброблений стан, дієтичні вимоги та виключення; коли спосіб приготування потребує певного відрубу або форми, обери конкретний сумісний сирий відруб чи форму названого виду за кулінарними знаннями моделі, а не за фіксованою таблицею відповідностей;
- не перетворюй сирий інгредієнт на готову страву й не додавай маринад, спеції, гарнір або інші складники рецепта, яких немає в погодженому плані; minimum_distinct_products лишається єдиною підставою декомпозувати одну позицію на кілька товарних потреб;
- у note коротко збережи вихідну роль або призначення продукту в погодженому меню, щоб перевірка кандидата бачила не лише каталожну назву;
- розділяй discovery та suitability: name і перші точні search_queries містять лише товарну ідентичність, вид та потрібний фізичний стан; майбутня страва, спосіб приготування й фрази призначення лишаються у note та використовуються тільки після отримання кандидатів для перевірки їхньої придатності;
- search_queries містить 2–6 впорядкованих коротких природних запитів українською для тієї самої купованої їжі: спочатку інша точна каталожна назва чи порядок слів зі збереженням названого виду й придатної форми, далі синонім або інший доречний відруб/форма, а останні один-два запити можуть шукати найближчу альтернативу з тією самою роллю; кожен запит називає їжу або товарну сімʼю — ніколи не людину, дію, інструкцію чи перевірку;
- для багатослівної товарної ідентичності серед перших трьох search_queries обовʼязково додай найкоротшу природну каталожну назву головного товару або товарної сімʼї; після двох близьких точних форм переходь до цього ширшого запиту, а не витрачай список на відмінки тих самих слів;
PROMPT;
    }

    private function preparationRepairNamingRules(): string
    {
        return <<<'PROMPT'
...
- розділяй discovery та suitability: name і перші точні search_queries містять лише товарну ідентичність, вид та потрібний фізичний стан; спосіб приготування і роль лишаються у note та перевіряються вже на кандидатах;
...
- для багатослівної товарної ідентичності серед перших трьох search_queries обовʼязково додай найкоротшу природну каталожну назву головного товару або товарної сімʼї; не заповнюй список лише граматичними формами тих самих слів;
PROMPT;
    }
...
У need_key дослівно повтори current_need.key. Не приймай рішення за іншу потребу, навіть якщо в coverage audit видно інші key.
...
Повне покриття кількості завжди пріоритетне. Якщо current_need.partial_stock_fallback=true, точні пошуки, практичні альтернативи й каталожні області вже вичерпано, а передані candidates мають лише додатний, але недостатній залишок. Тоді не роби retry або skip лише через нестачу: обери найкращий безпечний і сумісний candidate, передай повну бажану quantity, а застосунок обмежить її доступним максимумом і додасть видиме попередження. Така позиція покриває товарну роль, але coverage audit мусить мати enough_for_people=false.
...
- need_key мусить дослівно дорівнювати current_need.key; не ремонтуй рішення для іншої потреби;
...
PROMPT;
    ...
    {
        $currentNeedKey = data_get($context, 'current_need.key');

        if (is_string($currentNeedKey)
    ...
    private function decisionSchema(): array
    {
        return [
            'type' => 'object',
            'additionalProperties' => false,
            'required' => ['need_key', 'action', 'selected_product_id', 'query', 'quantity', 'reason', 'question', 'audit', 'allow_catalog_fallback', 'candidate_matches_required_product', 'safety_evidence', 'is_replacement'],
            'properties' => [
                'need_key' => ['type' => 'string'],
